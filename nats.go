...
Streaming Server Options:
    -cid, --cluster_id  <string>         Идентификатор кластера (по умолчанию: тестовый кластер)
    -st,  --store <string>               Тип хранилища: ПАМЯТЬ|ФАЙЛ|SQL
          --dir <string>                 Для типа хранилища ФАЙЛ это корневой каталог.
...
Streaming Server Clustering Options:
    --clustered <bool>                     Запустите сервер в кластере configuration (default: false)
    --cluster_node_id <string>             Идентификатор узла в кластере, если сохраненный идентификатор отсутствует (по умолчанию: случайный UUID)
    --cluster_bootstrap <bool>             Загрузите кластер, если существующего состояния нет, выбрав себя в качестве лидера (по умолчанию: false)
...
// использование распечатает параметры флага для сервера.
func usage() {
	fmt.Printf("%s\n", usageStr)
	os.Exit(0)
}
...
func main() {
	...
	// Заставьте сервер потоковой передачи настроить собственный обработчик сигналов.
	sOpts.HandleSignals = true
	// переопределить NoSigs для NATS, поскольку потоковая передача имеет собственный обработчик сигналов
	nOpts.NoSigs = true
	// Если для этой опции установлено значение true, регистратор не настроен.
	sOpts.EnableLogging = true
	// Это вызовет RunServerWithOpts, но в Windows его можно запустить как службу.
